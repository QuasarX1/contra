...
import h5py as h5
import os
from contra import ArrayMapping, ParticleType
from contra.io import SnapshotEAGLE, LineOfSightFileEAGLE

#TODO: configurable???
plot_point_alpha = 0.4
plot_point_size = 0.3

#contra-plot-eagle-recovered-los ./los_ids.hdf5 ~/EAGLE/L0100N1504/REFERENCE/data/ ~/EAGLE/L0100N1504/REFERENCE/data/los/ -f part_los_z0.010.hdf5 -i 99 -m
...
def main():
    ScriptWrapper(
        ...
        description = "Plot the positions of particles in an EAGLE line-of-sight along with the start and end positions of the matched particle.",
        parameters = ScriptWrapper.ParamSpec(
            ScriptWrapper.PositionalParam[str](
                name = "recovered-ids-file",
                description = "File containing recovered IDs."
            ),
            ScriptWrapper.PositionalParam[str](
                name = "data-directory",
            ...
            ScriptWrapper.PositionalParam[str](
                name = "do-file",
                short_name = "f",
                sets_param = "selected_file_name",
                description = "Line of Sight file to plot.\nSpecify the full name of the file (including extension) but not the path."
            ),
            ScriptWrapper.PositionalParam[int](
                name = "do-index",
                short_name = "i",
            ...
            ScriptWrapper.Flag(
                name = "centre",
                short_name = "c",
                sets_param = "centre_los",
                description = "Plot the line-of-sight in the centre of the box, applying any nessessary box-wrapping.",
            ),
            ScriptWrapper.Flag(
...
def __main(
            recovered_ids_file: str,
            data_directory: str,
            los_directory: str,
            selected_file_name: str,
            selected_los_index: int,
            centre_los: bool,
    ...
    selected_file = os.path.join(los_directory, selected_file_name)
    ...
    # Create an object for reading line-of-sight data

    los_file = LineOfSightFileEAGLE(os.path.join(los_directory, selected_file_name))
    los = los_file.get_sightline(selected_los_index)

    # Get the numbers of the snipshots that border the line of sight file's redshift

    snap_num_initial, snap_num_final = find_neighbouring_snapshots(los_file.z)

    previous_snap = SnapshotEAGLE(snapshot_filepaths_by_number[snap_num_initial])
    next_snap = SnapshotEAGLE(snapshot_filepaths_by_number[snap_num_final])

    # Read the recovered IDs

    recovered_ids = None
    with h5.File(recovered_ids_file, "r") as file:
        recovered_ids = file[selected_file_name][f"LOS{selected_los_index}"][:]

    L = next_snap.box_size[0].to("Mpc").value
    if centre_los:
        shift = (L / 2) - los.start_position.to("Mpc").value
        shift[los.direction == 1] = 0

    # Get helper info about the line of sight data axes

    coordinate_indexes = np.array([0, 1, 2], dtype = int)
    projection_axis_index = coordinate_indexes[los.direction == 1][0]

    # Read los data
    los_positions = los.positions_comoving.to("Mpc").value

    if use_memory_efficient_data_loading:

        # Get the displacements from the line-of-sight (delta in los axis must be ignored)

        los_displacements = calculate_wrapped_displacement(los.start_position.to("Mpc").value, los_positions, L)
        ...
        column_width = unyt_quantity(column_width, units = "Mpc")

        # Compute the region limits

        face_centre = los.start_position.to("Mpc")[np.where(~(los.direction > 0))[0]]
        ...
        previous_snap.restrict_data_comoving_loading_region(*restriction_region_args)
        next_snap.restrict_data_comoving_loading_region(*restriction_region_args)

    # Find the particles that appear in both snapshots

    Console.print_verbose_info("Loading particle ID data.")

    initial_sorter = ArrayMapping(previous_snap.get_IDs(ParticleType.gas), recovered_ids)
    final_sorter = ArrayMapping(next_snap.get_IDs(ParticleType.gas), recovered_ids)

    previous_snap_positions = initial_sorter(previous_snap.get_positions(ParticleType.gas).to("Mpc").value)
    next_snap_positions = final_sorter(next_snap.get_positions(ParticleType.gas).to("Mpc").value)

    # Optionally, shift the los to the centre of the box

    if centre_los:
        los_positions = np.mod(los_positions + shift, L)
        previous_snap_positions = np.mod(previous_snap_positions + shift, L)
        next_snap_positions = np.mod(next_snap_positions + shift, L)

    # Plot

    fig, axes = plt.subplots(2, 1, figsize = (14, 8), layout = "tight", sharex = True)

    axes[0].scatter(
        los_positions[:, projection_axis_index],
        los_positions[:, coordinate_indexes[los.direction != 1][0]],
        alpha = plot_point_alpha,
        s = plot_point_size,
        label = f"LoS (z={los_file.z:.3f})"
    )
    axes[0].scatter(
        previous_snap_positions[:, projection_axis_index],
        previous_snap_positions[:, coordinate_indexes[los.direction != 1][0]],
        alpha = plot_point_alpha,
        s = plot_point_size,
        label = f"Initial (z={previous_snap.z:.3f})"
    )
    axes[0].scatter(
        next_snap_positions[:, projection_axis_index],
        next_snap_positions[:, coordinate_indexes[los.direction != 1][0]],
        alpha = plot_point_alpha,
        s = plot_point_size,
        label = f"Final (z={next_snap.z:.3f})"
    )
    axes[1].scatter(
        los_positions[:, projection_axis_index],
        los_positions[:, coordinate_indexes[los.direction != 1][1]],
        alpha = plot_point_alpha,
        s = plot_point_size
    )
    axes[1].scatter(
        previous_snap_positions[:, projection_axis_index],
        previous_snap_positions[:, coordinate_indexes[los.direction != 1][1]],
        alpha = plot_point_alpha,
        s = plot_point_size
    )
    axes[1].scatter(
        next_snap_positions[:, projection_axis_index],
        next_snap_positions[:, coordinate_indexes[los.direction != 1][1]],
        alpha = plot_point_alpha,
        s = plot_point_size
    )
    axes[0].set_xlim((0, L))
    ...
    axes[1].set_ylim((0, L))
    axis_labels = ("X", "Y", "Z")
    axes[1].set_xlabel(f"{axis_labels[projection_axis_index]}-axis (Mpc)")
    axes[0].set_ylabel(f"{axis_labels[coordinate_indexes[los.direction != 1][0]]}-axis (Mpc)")
    axes[1].set_ylabel(f"{axis_labels[coordinate_indexes[los.direction != 1][1]]}-axis (Mpc)")
    axes[0].legend()
    plt.show()

    plt.figure(layout = "tight", figsize = (8, 8))
    plt.scatter(
        los_positions[:, coordinate_indexes[los.direction != 1][0]],
        los_positions[:, coordinate_indexes[los.direction != 1][1]],
        alpha = plot_point_alpha,
    ...
    plt.xlim((0, L))
    plt.ylim((0, L))
    plt.xlabel(f"{axis_labels[coordinate_indexes[los.direction != 1][0]]}-axis (Mpc)")
    plt.ylabel(f"{axis_labels[coordinate_indexes[los.direction != 1][1]]}-axis (Mpc)")
    plt.legend()
    plt.show()
