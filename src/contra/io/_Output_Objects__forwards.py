...
class ContraData(Struct):
    ...
    data: dict[ParticleType, dict[str, ParticleTypeDataset]] = TypedAutoProperty[dict[ParticleType, ParticleTypeDataset]](TypeShield(dict),#TODO: dict nested shield? (not cast!!!)
    ...
    @property
    def gas(self) -> dict[str, ParticleTypeDataset]|None:
        return self.data.get(ParticleType.gas, None)# self.data[ParticleType.gas] if ParticleType.gas in self.data else None
    @property
    def stars(self) -> dict[str, ParticleTypeDataset]|None:
        return self.data.get(ParticleType.star, None)# self.data[ParticleType.star] if ParticleType.star in self.data else None
    @property
    def black_holes(self) -> dict[str, ParticleTypeDataset]|None:
        return self.data.get(ParticleType.black_hole, None)# self.data[ParticleType.black_hole] if ParticleType.black_hole in self.data else None
    @property
    def dark_matter(self) -> dict[str, ParticleTypeDataset]|None:
        return self.data.get(ParticleType.dark_matter, None)# self.data[ParticleType.dark_matter] if ParticleType.dark_matter in self.data else None
...
#        print(f"CLOSED {self.__filepath}--------------------------------------------------------------------")
    ...
    def write_header(self, header: HeaderDataset) -> None:
        ...
        d.attrs["SimulationDirectory"] = header.simulation_directory
        d.attrs["NumberSearchedSnapshots"] = header.N_searched_snapshots
        d.attrs["UsesSnipshots"] = header.uses_snipshots
...
#        self.__file["Header"].attrs["NumberSearchedSnapshots"] = number_of_snapshots
    ...
    def write_particle_type_dataset(self, dataset: ParticleTypeDataset, overwrite: bool = False) -> None:
        if not self.is_open:
            raise IOError("File not open. Call open() first or use with statement.")
        if dataset.particle_type.common_hdf5_name not in self.__file:
            self.__file.create_group(dataset.particle_type.common_hdf5_name)
        if dataset.file_number in self.__file[dataset.particle_type.common_hdf5_name]:
            if overwrite:
                del self.__file[dataset.particle_type.common_hdf5_name][dataset.file_number]
            else:
                raise KeyError("Output dataset already exists. Specify \"overwrite\" to allow overwriting of existing datasets.")
        d = self.__file[dataset.particle_type.common_hdf5_name].create_group(dataset.file_number)
        d.attrs["Redshift"] = dataset.target_redshift
...
#        print(f"CLOSED {self.__filepath}--------------------------------------------------------------------")
    ...
    def read_header(self) -> HeaderDataset:
        ...
        s.simulation_directory = str(self.__file["Header"].attrs["SimulationDirectory"])
        s.N_searched_snapshots = int(self.__file["Header"].attrs["NumberSearchedSnapshots"])
        s.uses_snipshots = bool(self.__file["Header"].attrs["UsesSnipshots"])
    ...
    def read(self, include_stats: bool = True, alternate_simulation_data_directory: str|None = None) -> ContraData:
        ...
        if force_open:
            self.open()
        s = ContraData(directory_altered = alternate_simulation_data_directory is not None)
        s.header = self.read_header()
        loaded_data: dict[ParticleType, dict[str, ParticleTypeDataset]] = {}
        for p, check in {
            ParticleType.gas         : s.header.has_gas,
            ParticleType.dark_matter : s.header.has_dark_matter,
            ParticleType.star        : s.header.has_stars,
            ParticleType.black_hole  : s.header.has_black_holes
        }.items():
            if check:
                loaded_data[p] = {}
                for n in self._read_particle_type_dataset_keys(p):
                    loaded_data[p][n] = self.read_particle_type_dataset(p, n)
        s.data = loaded_data
        ...
        if alternate_simulation_data_directory is not None:
            s.header.simulation_directory = alternate_simulation_data_directory
        #TODO:
        #s.simulation_files = (FileTreeScraper_EAGLE if s.header.sim_type == "EAGLE" else FileTreeScraper_SWIFT)(s.header.simulation_directory)
        s.simulation_files = FileTreeScraper_EAGLE(s.header.simulation_directory)
        return s
